...
// Función para construir URLs correctas de imágenes
function buildImageUrl(imagePath) {
    if (!imagePath) return '';
    
    // Si la ruta ya es absoluta con protocolo, devolverla tal como está
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
        return imagePath;
    }
    
    // Si la ruta comienza con /static/, agregar /admin_blog si estamos en Apache
    if (imagePath.startsWith('/static/')) {
        // Detectar si estamos en servidor PHP incorporado (puerto específico) o Apache
        const isBuiltInServer = window.location.port && window.location.port !== '80' && window.location.port !== '443';
        
        if (isBuiltInServer) {
            // Servidor PHP incorporado - usar la ruta tal como está
            return window.location.origin + imagePath;
        } else {
            // Apache (XAMPP) - agregar /admin_blog
            return window.location.origin + '/admin_blog' + imagePath;
        }
    }
    
    // Para cualquier otra ruta, devolverla tal como está
    return imagePath;
}
...
async function loadBlogs(page = 1) {
    try {
        const response = await fetch(`/admin_blog/api/blogs?dashboard=true&page=${page}&per_page=6`);
...
async function saveBlog() {
    ...
    // Enviar los datos al backend PHP usando fetch
    try {
        const response = await fetch('/admin_blog/api/blogs', { method: 'POST', body: formData });
...
async function editBlog(id) {
    ...
    try{
        const response = await fetch(`/admin_blog/api/blogs/${id}`);
        const result = await response.json();
        ...
        if (result.ok){
            ...
            if(blog.imagen){
                const preview = document.getElementById('imagePreview');
                const previewImage = document.getElementById('previewImage');
                previewImage.src = buildImageUrl(blog.imagen);
                preview.style.display = 'block';
                console.log('Imagen cargada:', blog.imagen);
                console.log('URL construida:', buildImageUrl(blog.imagen));
...
async function updateBlog(id){
    ...
    try {
        const response = await fetch(`/admin_blog/api/blogs/${id}/update`,{method: 'POST', body:formData})
        const result = await response.json();
...
async function deleteBlog(id) {
    ...
    if (confirmDelete) {
        ...
        try {
            const response = await fetch(`/admin_blog/api/blogs/${id}`, { method: 'DELETE' });
            const result = await response.json();
        ...
        imageInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                if (!preview.dataset.existingImage) {
                    ...
                    previewImage.src = buildImageUrl(preview.dataset.existingImage);
                    preview.style.display = 'block';
                }
            }
        });
...
async function togglePopular(blogId, esPopular) {
    try {
        const response = await fetch(`/admin_blog/api/blogs/${blogId}/popular`, {
            method: 'PUT',
            headers: {
...
async function loadCategories() {
    try {
        const response = await fetch('/admin_blog/api/categorias');
        const result = await response.json();
